"""`fss_dino` against a seeded random ViT: references in, a mask and a map out, seeded in
both directions, and a round trip that scores identically. Accuracy needs pretrained
...
def test_references_in_a_mask_and_a_map_out(tmp_path: Path) -> None:
    ...
    with Image.open(infer_ctx.mask_path(10)) as opened:
        written = np.asarray(opened) > 0
    decided = probability != 0.5  # where neither score is positive, the map abstains
    assert np.array_equal(written[decided], (probability > 0.5)[decided])
