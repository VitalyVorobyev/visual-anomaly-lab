...
class ColorPrototypeModel(AnomalyModel):
    ...
    def _fit_class(
        self, name: str, parts: list[np.ndarray], label_key: str, ctx: TrainContext
    ) -> _Gaussian:
        ...
        ctx.metric(f"pixels_{'foreground' if name == 'the class' else 'background'}", len(chosen))
    ...
    def predict(self, images: Sequence[ImageRecord], ctx: InferContext) -> list[Prediction]:
        ...
        for index, record in enumerate(images):
            ...
            margin = self._foreground.log_likelihood(pixels) - self._background.log_likelihood(
                pixels
            )
            probability = (1.0 / (1.0 + np.exp(-np.clip(margin, -60.0, 60.0)))).reshape(
                array.shape[:2]
            )
