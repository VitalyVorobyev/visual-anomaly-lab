...
The plugin boundary allows ground truth through `TrainContext.targets`,
`TrainContext.label_targets` and `TrainContext.box_targets` alone. These are the
implementations: each reads an image's truth from what pinned it, verified, and applies the
image's pinned region transform so the target lines up with the prepared image the model was
given.
"""
...
class PreparedBoxTargets:
    """`BoxTargetProvider` over the labelled training images of one detection run."""

    def __init__(
        self,
        classes: tuple[str, ...],
        truths: dict[int, BoxTruth],
        region_build: PreparedRegionBuild,
    ) -> None:
        self._classes = classes
        self._truths = truths
    ...
    def boxes(self, image_id: int) -> list[TargetBox]:
        ...
        if truth is None:
            raise ClassTruthError(f"image {image_id} is not a labelled training image of this run")
        transform = self._region_build.transform_for(image_id)
        pinned = set(self._classes)
        found: list[TargetBox] = []
        # A class the run was not created with is nobody's truth, and an object the region
        # crop left out was never in the prepared image.
        for target in load_boxes(truth):
            if target.label_key not in pinned:
                continue
            box = transform.prepare_box(target.box)
            if box is not None:
                found.append(TargetBox(target.label_key, box))
        return found
