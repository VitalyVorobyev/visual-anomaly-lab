...
export function SampleTile({
  ...
  /**
   * Open the sample in place instead of navigating to its viewer — the reference studio's
   * rail focuses its stage. The tile is then a button, never a link.
   */
  onOpen?: (() => void) | undefined;
  /** Marks the sample the caller is showing, distinct from being selected. */
  ...
  const frame = cn(
    "flex h-full w-full flex-col overflow-hidden rounded-control border text-left transition-colors",
    selected
  ...
  return (
    <div className="relative">
      ...
      <span className="absolute top-1 left-1 z-10">
        <Checkbox
          checked={selected}
          aria-label={selectLabel ?? `Select ${sample.group_key}/${sample.external_id}`}
