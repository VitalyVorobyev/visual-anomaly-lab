...
Running this work inside the API process (see ADR-0003) is tempting and wrong. PyTorch with MPS is
not fork-safe and interacts badly with an event loop; a run that exhausts unified memory takes the
server down with it; Python does not reliably return large allocations to the OS, so a long-lived
server accumulates memory across runs; and a thread inside a C extension cannot be cancelled.
...
- **JSON-lines events**, one object per line. The parent persists what belongs in the database,
  tees the raw stream to a log file, and fans events out to subscribers. A new job kind is one
  handler; the queue and the protocol are kind-agnostic.
...
- **A thread or task inside the API process.** Rejected for the reasons above: no containment, no
  cancellation, no memory reclamation.
- **A worker pool running jobs concurrently.** More throughput in principle; on one machine with one
  accelerator it means memory contention, slower runs and timings that measure each other.
