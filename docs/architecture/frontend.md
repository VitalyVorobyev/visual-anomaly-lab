...
| **Run bar** | A draft run's primary action is **Train & score** (`then_score`, a queue follow-up — [the job system](jobs.md)), with **Train only** beside it; a trained run offers Retrain from scratch behind a confirmation, Continue for a resumable method, and Score & evaluate. The page follows whatever job is live, so the chained scoring job is picked up without a press, and a scoring run that lands while Overview is open moves the reader to Samples. | `POST /api/experiments/{id}/train`, `POST /api/experiments/{id}/infer` |
