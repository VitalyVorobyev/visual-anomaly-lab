...
| `explore` | `/api/explore`, `/api/images/…/explore`, `/api/images/…/explore/text` | which frozen encoders can run, and what one sees in an image: similarity, clusters, false colour; SAM 3's instances for a phrase; a mask as a candidate |
