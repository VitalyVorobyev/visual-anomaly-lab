...
"""Benchmark Dinomaly against PatchCore on two public VisA classes.
...
revision, then runs PatchCore and Dinomaly against those exact 392-pixel artifacts.  Each
experiment runs in a fresh process so peak RSS is comparable.
...
def _create_experiment(
    ...
    *,
    ...
    raw_config = dict(METHOD_CONFIGS[method])
    if method == "dinomaly_anomalib" and steps is not None:
        raw_config["max_steps"] = steps
    model_config = (
        get_model_class(method).config_model().model_validate(raw_config).model_dump(mode="json")
    )
    ...
    with connection(settings.db_path) as conn:
        experiment = experiments_repo.create_experiment(
            ...
            artifact_dir="",
            notes="Reproducible M11 Dinomaly public quality gate.",
        )
...
def _decision(categories: dict[str, Any]) -> dict[str, Any]:
    names = ("image_roc_auc", "pixel_roc_auc", "au_pro")
    candidate = {
        name: mean(
            value
            for legs in categories.values()
            if (value := _metric(legs["dinomaly_anomalib"], name)) is not None
        )
        for name in names
    }
    baseline = {
        name: mean(
            value
            for legs in categories.values()
            if (value := _metric(legs["patchcore_anomalib"], name)) is not None
        )
        for name in names
    }
    clears = (
        candidate["image_roc_auc"] >= QUALITY_FLOORS["mean_image_roc_auc"]
    ...
    return {
        ...
        "dinomaly_mean": candidate,
        "patchcore_mean": baseline,
        "dinomaly_minus_patchcore": {name: candidate[name] - baseline[name] for name in names},
        "quality_floors_clear": clears,
        "recommendation": (
            "promote dinomaly_anomalib as the transformer-reconstruction reference"
            if clears
            else "do not promote Dinomaly; investigate quality before changing the method set"
        ),
    }


def _run(data_dir: Path, categories: tuple[str, ...], steps: int | None) -> int:
    ...
    if steps is not None:
        report["methods"]["dinomaly_anomalib"]["max_steps"] = steps
    log_path = data_dir / "gate.log"
    ...
    report["decision"] = _decision(report["categories"])
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        choices=tuple(
            spec.key.split(":", 1)[1]
            for pack in pack_specs(_settings(Path("/tmp/dinomaly-gate-catalog")))
            if pack.key == "visa"
    ...
    parser.add_argument(
        "--dinomaly-steps",
        type=int,
        help="Override 5000 only for a smoke run; the recorded quality gate uses 5000.",
    )
    ...
    if len(categories) < 2 and args.dinomaly_steps is None:
        parser.error("the quality gate requires at least two VisA classes")
    return _run(args.data_dir, categories, args.dinomaly_steps)
