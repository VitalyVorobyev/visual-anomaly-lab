...
image after a real fit. The gate lends the format for its own run to a plugin that implements the protocol
without declaring it, so the declaration lands with the verdict, not before it. If operator coverage is
missing, leave export unsupported; a truthful absence is better than a button that changes semantics.
