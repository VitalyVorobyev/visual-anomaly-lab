# Summary

- [Visual Anomaly Lab](introduction.md)
- [Quick start](quick-start.md)
- [Core concepts](concepts.md)
- [A new dataset, end to end](new-dataset.md)

# Workflows

- [Import and registration](import.md)
- [Annotation](annotation.md)
- [Object regions and preprocessing](preprocessing.md)
- [Model assets and offline replay](model-assets.md)
- [Distilling an EfficientAD teacher](teacher.md)
- [Experiments and jobs](experiments.md)
- [Inference, evaluation, and comparison](evaluation.md)
- [Portable ONNX deployment](deployment.md)

# Methods and evidence

- [Supported methods](generated/methods.md)
- [Choosing a method](model-selection.md)
- [Public benchmarks](generated/benchmarks.md)
- [Interpreting metrics](metrics.md)

# Extending the workbench

- [Add a model](add-model.md)
- [Add preprocessing or localization](add-preprocessing.md)
- [Add an import adapter](add-adapter.md)
- [System design](system-design.md)
- [Development and verification](development.md)
